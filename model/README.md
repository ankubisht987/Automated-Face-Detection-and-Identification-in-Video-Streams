...
A Streamlit-based web application that detects and recognizes faces in videos using RetinaFace for face detection and ArcFace for face recognition.

## Features

- **Face Detection**: Uses RetinaFace for accurate face detection in video frames
- **Face Recognition**: Employs ArcFace (via InsightFace) for high-accuracy face recognition
...
- **`FaceDetector`**: Handles face detection using RetinaFace
- **`FaceRecognizer`**: Manages face recognition using ArcFace/InsightFace
- **`VideoProcessor`**: Coordinates video processing and face detection workflow

### Face Recognition Pipeline
1. **Detection**: RetinaFace identifies face regions in each frame
2. **Extraction**: ArcFace generates 512-dimensional face embeddings
